...
- UNet: `sbatch --partition=gputest --gres=gpu:v100:1 train/slurm/train_big_unet_att.sh`
- U-AFNO: `sbatch --partition=gputest --gres=gpu:v100:1 train/slurm/train_big_uafno.sh`
- FNO: `sbatch train/slurm/train_fno.sh` (already gputest-ready)
...
- Chain wavelet → train automatically: `python tools/chain_wavelet_and_train.py --wavelet-sbatch datapipes/wavelet_weights_a10000b50000.sh --train-sbatch train/slurm/train_big_uafno.sh --train-config configs/train/uafno_wavelet.yaml`. This waits for the wavelet job to finish, then submits training.
...
- Request GPUs/CPUs that match torchrun: `--gres=gpu:v100:2` pairs with `--nproc_per_node=2`; keep `--cpus-per-task≈10×num_gpus` (e.g., 20 for 2 GPUs, 40 for 4 GPUs). Examples: `train/slurm/train_flowmatch_uafno_full_2gpu.sh` (2×V100) or bump to 4 GPUs by setting `--gres=gpu:v100:4`, `--cpus-per-task=40`, `--nproc_per_node=4`.
...
- Quick DDP check on a single node: `python -m models.train.core.ddp_multi_node_check --backend nccl --world-size 2 --nproc-per-node 2` inside the allocation; for Slurm, submit `train/slurm/test_parallel_multinode.sh` to exercise NCCL/env wiring before heavy runs.
