...
This directory now mirrors only the rapid solidification workflow, with the Python package renamed to `models/` (formerly `pf_surrogate_modelling`). Key layout:
...
- `train/slurm/` – ready-to-run Slurm launchers that point at `models/train/core/train.py`. The top-level `train/core/*.csv` files are legacy throughput logs only.
- `train/slurm/benchmark_flowmatch_uafno_scaling.sh` – gputest scaling smoke for the ~202M UAFNO (1/2/4/8 GPUs torchrun sweep).
...
- `NEXT_STEPS.md` – checklist of follow-up runs (smoke tests, datapipes, visuals).
- `ENVIRONMENT.md` – Puhti instructions for rebuilding the Tykkÿ/conda container (`physics_ml/`).
- `PUHTI_PARTITIONS.md` – cheat sheet for Puhti partitions (gputest/gpu/gpusmall) and GPU request syntax.
...
| `train/slurm/` | Puhti-ready SLURM scripts. Test and smoke scripts default to `gputest` with `--gres=gpu:v100:1`; edit only when queueing long `gpu` jobs. |
| `train/slurm/benchmark_flowmatch_uafno_scaling.sh` | Torchrun scaling sweep (1/2/4/8 GPUs) for the ~202M UAFNO depth-8 config on gputest (15 min cap). |
...
2. **Run smoke training** – `python -m models.train.core.train -c configs/train/train_smoke.yaml` or `sbatch train/slurm/train_smoke.sh` (≤15 min on `gputest`, `gpu:v100:1`, `cpus-per-task=8`).
...
5. **Diffusion smoke (residual DDPM)** – `sbatch train/slurm/train_diffusion.sh` (runs `experiments/diffusion_prototype/src/train_ddpm_residual.py` on the placeholder HDF5, GPU-friendly via `gputest`).
...
- `train/slurm/test_parallel_multinode.sh` submits a Puhti `gputest` job (2 nodes × 4 V100 GPUs, 15‑minute limit), runs the DDP check, and then executes `train/core/parallel_solid_data.py` across all 8 GPUs using the placeholder HDF5 dataset before emitting `seff`/`sacct` diagnostics.
- Submit via `sbatch train/slurm/test_parallel_multinode.sh`, then `squeue -j <jobid>` to monitor. The output file `logs/slurm/%x_%j.out` captures both steps so you can confirm hostnames, ranks, and throughput.
...
- For GPU utilisation tooling (`seff`, `sacct`, live `nvidia-smi`, Nsight profiling, scaling tests), refer to `GPU_MONITORING.md`.
...
If any step fails, re-launch Codex directly on the cluster login node with your user and re-run the checklist before attempting the longer scripts in `train/slurm/`.
...
4. **Tests before pushes** – run `PYTHONPATH=models python -m pytest tests/test_backbones_rs.py -q` (fast) or `tests -q` (full CPU), and on GPU boxes submit `train/slurm/train_smoke.sh` for a quick sanity.
5. **Future CI** – mirror the smoke test (`pytest -q tests`) and debug train command in GitHub Actions once secrets are set up, using the Tykkÿ container recipe in `ENVIRONMENT.md` for reproducibility.
