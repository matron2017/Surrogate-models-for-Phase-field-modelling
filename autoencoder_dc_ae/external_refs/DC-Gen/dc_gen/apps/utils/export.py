...
def export_onnx(
    model: nn.Module, export_path: str, sample_inputs: Any, simplify: bool = True, opset: int = 11, large: bool = False
) -> None:
    ...
    if large:
        ...
        with torch.inference_mode():
            torch.onnx.export(model, sample_inputs, export_path, opset_version=opset)
    else:
        buffer = io.BytesIO()
        with torch.no_grad():
