...
__all__ = ["CosineLRwithWarmup", "ConstantLRwithWarmup", "WSDCosineLR"]
...
class CosineLRwithWarmup(torch.optim.lr_scheduler._LRScheduler):
    ...
    def get_lr(self) -> list[float]:
        if self.last_epoch < self.warmup_steps:
            ...
            decay_steps = decay_steps[idx + 1] - decay_steps[idx]
            return [0.5 * base_lr * (1 + math.cos(math.pi * current_steps / decay_steps)) for base_lr in self.base_lrs]
...
class WSDCosineLR(torch.optim.lr_scheduler._LRScheduler):
    def __init__(
        self,
        optimizer: torch.optim.Optimizer,
        warmup_steps: int,
    ...
    def get_lr(self) -> list[float]:
        ...
        elif self.last_epoch < self.stable_steps:
            return self.base_lrs
        else:
            current_steps = self.last_epoch - self.stable_steps
