...
class DPM_Solver:
    ...
    def singlestep_dpm_solver_second_update(
        ...
        if self.algorithm_type == "dpmsolver++":
            ...
            if solver_type == "dpmsolver":
                x_t = (
                    ...
                    * (
                        model_s1 - model_s
                    )  # TODO: delete it after test. (0.5 * h / (lambda_s1 - lambda_s)) * (sigma_t * phi_1) * (model_s1 - model_s)
                )
            elif solver_type == "taylor":
                x_t = (
                    torch.exp(log_alpha_t - log_alpha_s) * x
        ...
        if return_intermediate:
            self.intermediates.append(x_s1)
    ...
    def singlestep_dpm_solver_third_update(
        ...
        if return_intermediate:
            self.intermediates.append(x_s1)
            self.intermediates.append(x_s2)
    ...
    def singlestep_dpm_solver_update(
        ...
        Returns:
            x_t: A pytorch tensor. The approximated solution at time `t`.
        """
        prediction_type = "data_prediction" if self.algorithm_type == "dpmsolver++" else "noise_prediction"
        # print(f"Single step update: from timestep {s} to {t} | use order: {order} | solver type: {solver_type} | prediction type: {prediction_type}")
        if order == 1:
    ...
    def multistep_dpm_solver_update(self, x, model_prev_list, t_prev_list, t, order, solver_type="dpmsolver"):
        ...
        s = t_prev_list[-1]
        prediction_type = "data_prediction" if self.algorithm_type == "dpmsolver++" else "noise_prediction"
        # print(f"Multi step update: from timestep {s} to {t} | use order: {order} | solver type: {solver_type} | prediction type: {prediction_type}")
        if order == 1:
            ...
            if method == "adaptive":
                ...
                for step, order in enumerate(orders):
                    ...
                    x = self.singlestep_dpm_solver_update(
                        x, s, t, order, return_intermediate=return_intermediate, solver_type=solver_type, r1=r1, r2=r2
                    )
                    if self.correcting_xt_fn is not None:
                        x = self.correcting_xt_fn(x, t, step)
                    if return_intermediate:
