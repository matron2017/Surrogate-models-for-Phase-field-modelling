# Phase‑field HDF5 dataset THIS DATA CREATION IS DONE IN MAHTI, NOTE THIS IF WORKING IN PUHTI CURRENTLY

This README explains what the data‑build scripts produce, how tensors and metadata are shaped, and how to read them for modelling. See `configs/data/*.yaml` for the build descriptors wired into `datapipes/*.sh` (export `PF_DATA_CONFIG` accordingly).
...
accelerators: {max_gpus: 2}   # >1 enables multi-GPU VTU decoding
...
sim_manifest.json            # single lookup combining map + metadata
simulation_train.h5
simulation_val.h5
simulation_test.h5
```
...
### 3.4 `sim_manifest.json`

Combined convenience manifest keyed by original simulation name. Each entry stores `group_id` plus the same metadata blobs found in `sim_meta.json`, so downstream scripts can load a single JSON to resolve both IDs and physical parameters.
...
Mahti queueing reminders:

- `gpusmall` allows **at most 2× A100 GPUs** and 64 CPU cores per job. Queue times are reasonable, so we run the dataset build here (usually 1 GPU + 32 CPUs).
- `gpumedium` only accepts jobs that occupy **all four GPUs per node** and typically sits in the queue for hours; avoid it unless you truly need ≥4 GPUs.
- `gputest` has a 15‑minute cap — useful only for micro‑runs.
- Stay within the fair‑use guideline of ≤32 CPU cores per GPU.

A batch script tailored for `gpusmall`:

```bash
#!/bin/bash
#SBATCH --job-name=pf_h5_build
#SBATCH --account=project_2008261
#SBATCH --partition=gpusmall
#SBATCH --gres=gpu:v100:1          # Puhti V100 nodes (request ≤2 GPUs)
...
/scratch/project_2008261/physics_ml/bin/python3.11 build_hdf5_datasets.py
```
