...
- Δt z-score based on TRAINING pairs only
...
def load_yaml_config() -> Dict[str, Any]:
    default_cfg = "/scratch/project_2008261/rapid_solidification/configs/data/phase_field_data.yaml"
...
def main():
    ...
    if os.path.exists(norm_path):
        ...
        if norm.get('method_images') == 'zscore' and len(norm.get('channel_mean', [])) == len(FIELDS):
            ...
            dt_mean = float(norm.get('delta_t_mean', 0.0))
            dt_std  = float(norm.get('delta_t_std',  1.0))
            eps_img = float(norm.get('eps_images',  eps_img))
            eps_tg  = float(norm.get('eps_thermal', eps_tg))
            eps_dt  = float(norm.get('eps_delta_t', eps_dt))
            have_cache = True

    if not have_cache:
        imgs_accum: List[np.ndarray] = []
        tg_vals: List[float] = []
        dt_vals: List[np.ndarray] = []
        ...
        for sim in splits.get('train', []):
            ...
            delta_euler = (ptime[:, 1] - ptime[:, 0]).astype(np.float64)
            dt_vals.append(delta_euler * eff_dt)
        ...
        dt_all = np.concatenate(dt_vals, axis=0) if dt_vals else np.array([1.0], dtype=np.float64)
        dt_mean = float(np.mean(dt_all)); dt_std = float(np.std(dt_all)) if dt_all.size > 1 else 1.0
        ...
        logger.info(f"[norm] training-only zscore: ch_mean={ch_mean} ch_std={ch_std} "
                    f"tg_mean={tg_mean:.6g} tg_std={tg_std:.6g} dt_mean={dt_mean:.6g} dt_std={dt_std:.6g}")
    ...
    for split, sim_list in splits.items():
        ...
        with h5py.File(fout, mode) as h5f:
            ...
            h5f.attrs['delta_t_mean'] = float(dt_mean)
            h5f.attrs['delta_t_std']  = float(dt_std)
            h5f.attrs['zscore_eps_images']   = float(eps_img)
            h5f.attrs['zscore_eps_thermal']  = float(eps_tg)
            h5f.attrs['zscore_eps_delta_t']  = float(eps_dt)
            ...
            for sim in sim_list:
                ...
                # Δt: compute physical and z-score using training stats
                eff_dt = float(prm.get('dt', 1.0)) * TAU0
                delta_euler = (ptime[:, 1] - ptime[:, 0]).astype(np.int64)
                dt_phys = delta_euler.astype(np.float64) * eff_dt
                dt_norm = ((dt_phys - dt_mean) / (tg_std if False else (dt_std if dt_std > 0 else eps_dt))).astype(np.float32) if use_dt_norm \
                          else dt_phys.astype(np.float32)
                ...
                if 'images' in grp:
                    ...
                    grp['pairs_dt_euler'].resize((grp['pairs_dt_euler'].shape[0] + delta_euler.shape[0],)); grp['pairs_dt_euler'][-delta_euler.shape[0]:] = delta_euler
                    grp['pairs_dt'].resize((grp['pairs_dt'].shape[0] + dt_phys.shape[0],)); grp['pairs_dt'][-dt_phys.shape[0]:] = dt_phys
                    grp['pairs_dt_norm'].resize((grp['pairs_dt_norm'].shape[0] + dt_norm.shape[0],)); grp['pairs_dt_norm'][-dt_norm.shape[0]:] = dt_norm
                else:
                    ...
                    grp.create_dataset('pairs_dt_euler', data=delta_euler, maxshape=(None,), compression='gzip', chunks=(min(4096, len(delta_euler)),))
                    grp.create_dataset('pairs_dt',       data=dt_phys,     maxshape=(None,), compression='gzip', chunks=(min(4096, len(dt_phys)),))
                    grp.create_dataset('pairs_dt_norm',  data=dt_norm,     maxshape=(None,), compression='gzip', chunks=(min(4096, len(dt_norm)),))
                ...
                grp.attrs['norm_type_delta_t']     = 'zscore' if use_dt_norm else 'none'
                grp.attrs['zscore_eps_delta_t']    = float(eps_dt)
                grp.attrs['delta_t_mean']          = float(dt_mean)
                grp.attrs['delta_t_std']           = float(dt_std)
                ...
                # Meta
                sim_meta[sim_map.get(sim, f'sim_????')] = {
                    'original_name'  : sim,
