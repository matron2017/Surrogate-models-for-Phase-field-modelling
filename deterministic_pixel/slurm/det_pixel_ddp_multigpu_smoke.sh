...
RUN_TAG=${RUN_TAG:-smoke_det_unet_afno_controlxs_wavelet_512_ddp1n}
...
echo "[det-pixel-ddp-smoke-1n] cfg=$TMP_CFG"
echo "[det-pixel-ddp-smoke-1n] out_dir=$OUT_DIR"
echo "[det-pixel-ddp-smoke-1n] world_size=$WORLD_SIZE batch_per_rank=$BATCH_PER_RANK accum=$ACCUM_STEPS effective_batch=$((WORLD_SIZE * BATCH_PER_RANK * ACCUM_STEPS))"
nvidia-smi || true
