#!/bin/bash
# UniDB bridge full training — 3 nodes × 4 GPUs = 12 ranks, 300M+ backbone
# Effective global batch: 12 ranks × 6 per_gpu = 72 (same as det pixel)
#SBATCH --job-name=bridge_unidb
...
CONFIG=${CONFIG:-${ROOT}/diffusion_bridge/configs/unidb_bridge_pde_512_big.yaml}
NODES=${SLURM_NNODES:-3}
GPUS_PER_NODE=4

# Resolve master IP (same pattern as det pixel launcher)
...
echo "======================================================="
echo " UniDB bridge FULL  job=${SLURM_JOB_ID}"
...
echo "UniDB bridge training DONE"
