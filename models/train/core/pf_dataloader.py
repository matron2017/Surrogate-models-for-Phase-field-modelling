...
class PFPairDataset(Dataset):
    ...
    Returns per item:
      {
        ...
        "cond":   (2,) float32  # [t_abs_norm, G_norm]
        "gid":    str,
        "pair_index": int,
    ...
    def __getitem__(self, idx: int):
        ...
        # Scalars (float32): absolute time (normalised) and thermal gradient
        eff_dt = float(g.attrs.get("effective_dt", g.file.attrs.get("effective_dt", 1.0)))
        if "time_phys" in g:
            t_abs = float(g["time_phys"][j])
        elif "pairs_time" in g:
            t_abs = float(g["pairs_time"][k][1] * eff_dt)
        elif "times" in g:
            t_abs = float(g["times"][j] * eff_dt)
        else:
            raise KeyError("Expected absolute time datasets ('time_phys', 'pairs_time', or 'times').")

        t_mean, t_denom, _ = self.time_stats.get(gid, (0.0, 1.0, eff_dt))
        t_norm = (t_abs - t_mean) / t_denom
        ...
        cond = torch.tensor([t_norm, G_n], dtype=xi.dtype)  # (2,)
