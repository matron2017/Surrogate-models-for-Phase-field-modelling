...
CFG_BASE=${CFG_BASE:-$STACK_ROOT/configs/pf_surrogates/big/train_bridge_frac_unet_afno_controlxs_pixel512_big.yaml}
...
RUN_TAG=${RUN_TAG:-bridge_frac_unet_afno_controlxs_pixel512_${STAMP}}
cd "$PF_ROOT"
ACC=$((GLOBAL_BATCH/(RUN_TASKS*BATCH_PER_GPU)))
JOBID=$(sbatch -A project_462001338 -p "$PARTITION" -N "$NODES" --ntasks-per-node=1 --gpus-per-node="$GPUS_PER_NODE" --cpus-per-task="$CPUS_PER_TASK" -t "$TIME_LIMIT" -J brgfracpx \
