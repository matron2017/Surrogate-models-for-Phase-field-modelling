...
CFG=$ROOT/configs/pf_surrogates/smoke/train_smoke_bridge_unidb_t25_onepair.yaml
RUN_BRIDGE_PREFLIGHT=${RUN_BRIDGE_PREFLIGHT:-1}
BRIDGE_PREFLIGHT_STRICT=${BRIDGE_PREFLIGHT_STRICT:-0}
STAMP=$(date -u +%Y%m%dT%H%M%SZ)
RUN_TAG=smoke_bridge_unidb_t25_onepair_${STAMP}
...
cd "$PF"
JOBID=$(sbatch -A project_462001338 -p dev-g -N 1 --ntasks-per-node=1 --gpus-per-node=1 --cpus-per-task=7 -t 00:25:00 -J ov1brg \
  --export=ALL,RUN_TAG="$RUN_TAG",CFG_BASE="$CFG",MODE=strong,RUN_TASKS=1,GLOBAL_BATCH=1,BATCH_PER_GPU=1,EPOCHS=1,STEPS_PER_EPOCH=220,USE_VAL=1,DETERMINISTIC_OVERRIDE=0,CLEAR_RESUME=1,RUN_BRIDGE_PREFLIGHT="$RUN_BRIDGE_PREFLIGHT",BRIDGE_PREFLIGHT_STRICT="$BRIDGE_PREFLIGHT_STRICT" \
...
EVAL_JSON="$OUT_DIR/overfit_onepair_eval_suite.json"
EVAL_DIR="$OUT_DIR/overfit_onepair_eval_panels"
...
singularity exec --rocm "$AIF_IMG" "$AIF_PY" "$ROOT/train_utils/smoke/eval_overfit_unidb_suite.py" --ckpt "$CKPT" --split val --nfe 25 --max-items 1 --device cuda --seed 1 --out-json "$EVAL_JSON" --out-dir "$EVAL_DIR"
EOF
)

EJOB=$(sbatch -A project_462001338 -p dev-g -N 1 --ntasks-per-node=1 --gpus-per-node=1 --cpus-per-task=7 -t 00:15:00 -J ev1brg \
  --output "$PF/logs/slurm/ev1brg_%j.out" --wrap "$EWRAP" | awk '{print $4}')
