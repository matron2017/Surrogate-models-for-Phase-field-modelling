#!/bin/bash
# Resume AE latent PSGD run from latest checkpoint with a short 10h continuation:
...
#SBATCH --job-name=ae_latent_psgd_resume10h_lr3e6_70ep
...
NEW_OUT=${ROOT}/runs/ae_latent_lola_big_64_1024_psgd_uncached_freq1_12g_latent32_nowavelet_rightclean_fixed34_gradshared_b40_precond64_p128_resume10h_lr3e6_to1e6_70ep
BOOTSTRAP_DIR=${NEW_OUT}/bootstrap_from_prev_run
TMP=${SLURM_TMPDIR:-/tmp}/ae_resume_${SLURM_JOB_ID}
mkdir -p "${TMP}" "${BOOTSTRAP_DIR}"

CFG_RESUME=${TMP}/config_ae_resume10h_lr3e6_to1e6_70ep.yaml
...
state = torch.load(src_last, map_location="cpu")
...
shutil.copy2(src_last, backup_last)
...
manifest = {
    ...
    "purpose": "10h AE continuation from latest epoch with controlled LR schedule",
...
echo "=== Launching AE resume job (10h, +70 epochs, LR 3e-6 -> 1e-6) ==="
