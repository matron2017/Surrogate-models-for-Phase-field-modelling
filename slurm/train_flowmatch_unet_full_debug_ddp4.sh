#!/bin/bash
#SBATCH --job-name=flowmatch_unet_full_debug_ddp4
...
CFG=${ROOT}/configs/train/train_flowmatch_unet_full_debug.yaml

MASTER_HOST=$(scontrol show hostnames "${SLURM_JOB_NODELIST}" | head -n 1)
MASTER_ADDR=$(getent ahostsv4 "${MASTER_HOST}" | awk 'NR==1 {print $1}')
MASTER_PORT="${MASTER_PORT:-29500}"

echo "=== Launching 4-GPU flow-matching UNet (debug, gputest) ==="
