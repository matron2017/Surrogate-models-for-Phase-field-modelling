#!/bin/bash
# 24-hour latent SFM flow-matching run (1 node x 4 GPUs), UNet AFNO bottleneck.

#SBATCH --job-name=latent_unet_sfm24h_rdbm_afno8_b64_1n4g
...
CFG=${CFG:-${ROOT}/configs/train/train_flowmatch_unet_thermal_latentpsgd_e279_gpu24h_1n4g_b64_rdbmres_afno8_sfm_latent_long_v1.yaml}
...
echo "[sfm-train] host=$(hostname) job=${SLURM_JOB_ID:-na}"
echo "[sfm-train] cfg=${CFG}"
nvidia-smi || true

# Preflight is fast and catches config-contract regressions early.
${PY} scripts/bridge_preflight_check.py --config "${CFG}"
