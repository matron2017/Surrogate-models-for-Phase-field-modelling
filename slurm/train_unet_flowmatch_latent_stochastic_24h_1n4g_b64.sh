#!/bin/bash
# 24-hour latent flow-matching run (1 node x 4 GPUs), unet AFNO bottleneck.

#SBATCH --job-name=latent_unet_flowmatch24h_rdbm_afno8_b64_1n4g_sto
...
CFG=${CFG:-${ROOT}/configs/train/train_flowmatch_unet_thermal_latentpsgd_e279_gpu24h_1n4g_b64_rdbmres_afno8_stochastic.yaml}
RESUME=${RESUME:-}
...
${PY} -m torch.distributed.run --nproc_per_node=4 -m models.train.core.train -c "${CFG}" "${RESUME_FLAG[@]}"
