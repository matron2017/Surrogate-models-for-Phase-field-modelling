#!/bin/bash
# 5-hour latent flow/diffusion run (1 node x 4 GPUs), effective batch size 80.

#SBATCH --job-name=latent_uvit_thermal_5h_b80
...
CFG=${CFG:-${ROOT}/configs/train/train_flowmatch_uvit_thermal_sourceanchored_latentbest213_gpu5h_b80.yaml}
...
echo "[latent-5h] host=$(hostname) job=${SLURM_JOB_ID:-na}"
echo "[latent-5h] cfg=${CFG}"
nvidia-smi || true
...
eff = batch * 4 * acc
print(f"[latent-5h] guard OK: batch_per_rank={batch} world_size=4 accumulation={acc} effective_batch={eff}")
if eff != 80:
