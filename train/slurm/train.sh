...
# ---- Alternatives ----
# 2 GPUs (default: keep --cpus-per-task=20 and OMP_NUM_THREADS=10 to stay at 10 CPU cores/GPU)
srun --mem=0 --cpu-bind=cores --hint=nomultithread \
  ${PY} -m torch.distributed.run --standalone --nnodes=1 --nproc_per_node=2 \
  ${TRAIN_SCRIPT} -c ${CFG}
##
#1 GPU (drop to --cpus-per-task=10 and OMP_NUM_THREADS=10)
#srun --mem=0 --cpu-bind=cores --hint=nomultithread \
