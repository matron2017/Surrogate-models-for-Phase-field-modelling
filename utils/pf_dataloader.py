...
class PFPairDataset(Dataset):
    ...
    def __init__(
        ...
        self.normalize_thermal = bool(normalize_thermal)
        self.thermal_norm_source = str(thermal_norm_source).lower()
        self._norm_cache: Dict[str, Tuple[torch.Tensor, torch.Tensor, float, bool]] = {}
        self._thermal_norm_cache: Dict[str, Tuple[torch.Tensor, torch.Tensor, float, bool]] = {}
        ...
        with h5py.File(self.h5_path, "r") as h5:
            ...
            if self.normalize_thermal:
                if "thermal_field_channel_mean" in h5.attrs and "thermal_field_channel_std" in h5.attrs:
                    mean = torch.tensor(h5.attrs["thermal_field_channel_mean"], dtype=torch.float32)
                    std = torch.tensor(h5.attrs["thermal_field_channel_std"], dtype=torch.float32)
                    eps = float(h5.attrs.get("zscore_eps_thermal_field", h5.attrs.get("zscore_eps_thermal", 1e-6)))
                    already = str(h5.attrs.get("thermal_field_norm", "")).lower() == "zscore"
                    file_thermal_norm = (mean, std, eps, already)
            for gid in sorted(h5.keys()):
                ...
                if file_thermal_norm is not None and self.thermal_norm_source == "file":
                    self._thermal_norm_cache[gid] = file_thermal_norm
    ...
    def _get_thermal_norm_stats(self, gid: str, g: h5py.Group) -> Optional[Tuple[torch.Tensor, torch.Tensor, float, bool]]:
        if not self.normalize_thermal:
            return None
        cached = self._thermal_norm_cache.get(gid)
        if cached is not None:
            return cached
        mean = std = None
        eps = 1e-6
        already = False
        if self.thermal_norm_source == "group":
            if "thermal_field_channel_mean" in g.attrs and "thermal_field_channel_std" in g.attrs:
                mean = torch.tensor(g.attrs["thermal_field_channel_mean"], dtype=torch.float32)
                std = torch.tensor(g.attrs["thermal_field_channel_std"], dtype=torch.float32)
                eps = float(g.attrs.get("zscore_eps_thermal_field", g.attrs.get("zscore_eps_thermal", 1e-6)))
                already = str(g.attrs.get("thermal_field_norm", g.attrs.get("norm_type_thermal", ""))).lower() == "zscore"
        elif self.thermal_norm_source == "file":
            if "thermal_field_channel_mean" in g.file.attrs and "thermal_field_channel_std" in g.file.attrs:
                mean = torch.tensor(g.file.attrs["thermal_field_channel_mean"], dtype=torch.float32)
                std = torch.tensor(g.file.attrs["thermal_field_channel_std"], dtype=torch.float32)
                eps = float(g.file.attrs.get("zscore_eps_thermal_field", g.file.attrs.get("zscore_eps_thermal", 1e-6)))
                already = str(g.file.attrs.get("thermal_field_norm", "")).lower() == "zscore"
        if mean is None or std is None:
            return None
        stats = (mean, std, eps, already)
        self._thermal_norm_cache[gid] = stats
        return stats
    ...
    def __getitem__(self, idx: int):
        ...
        if self.add_thermal:
            thermal_stats = self._get_thermal_norm_stats(gid, g)
            ...
            if self.normalize_thermal:
                if thermal_stats is None:
                    raise RuntimeError(
                        "normalize_thermal=True but no thermal_field_channel_mean/thermal_field_channel_std found in HDF5 attrs."
                    )
                mean_t, std_t, eps_t, thermal_already = thermal_stats
                if self.normalize_force or not thermal_already:
                    ti_map = self._apply_normalize(ti_map, mean_t, std_t, eps_t)
                    if self.thermal_on_target:
                        tj_map = self._apply_normalize(tj_map, mean_t, std_t, eps_t)
