...
def main():
    ...
    with h5py.File(h5_test_path, "r") as h5:
        ...
        for batch in dl:
            ...
            for i in range(B):
                ...
                # Per-pair metadata
                t_abs, z_t = _pair_time_values(grp, k)
    ...
    for idx, rec in enumerate(examples):
        ...
        t_abs = rec["meta"]["time_phys"]; zt = rec["meta"]["z_time"]; G = rec["meta"]["G_raw"]; zG = rec["meta"]["z_G"]
        meta_str = f"gid={gid}  k={k}  t{tf}->{tt}  stride={s}  t_abs={t_abs:.3e}  zt={zt:.3f}  G={G:.3e}  zG={zG:.3f}"
        for c in range(C):
            _save_single_grid(
